# -*- coding: utf-8 -*-
"""Export resume-friendly RU wisdom-story illustration prompts from DOCX.

Does not call any image API. Use when regenerating
`ru/wisdom-stories/illustrations/{stem}.webp` so on-image Russian text matches
the story exactly.

  python tools/export_ru_illustration_prompts.py --pending --limit 5
  python tools/export_ru_illustration_prompts.py --stem baklava
  python tools/export_ru_illustration_prompts.py --batch B1 --out docs/reviews/ru-illu-prompts-B1.json
"""
...
PROGRESS = ROOT / "docs" / "reviews" / "ru-illustration-regen-progress.json"
DOCX_DIR = ROOT / "ru" / "wisdom-stories"
...
STYLE = (
    ...
    "No English, Azerbaijani, Kyrgyz, or Turkish text. Every letter of Russian "
    "text on the image must match the Exact quotes below character-for-character "
    "(spelling, punctuation, capitalization, ё vs е). Do not paraphrase, "
    "translate, or invent dialogue."
)
...
def build_prompt(stem: str) -> dict:
    ...
    dialogue_hint = (
        ...
        "(lines that begin with — or «…»). Put narrative captions only when needed, "
        "copied verbatim from BODY. Moral banner must use MORAL exactly. "
        "Do not invent shop signs, price tags, or labels unless that exact Russian "
        "string appears in BODY."
    )
    prompt = "\n\n".join(
        [
            STYLE,
    ...
    return {
        ...
        "moral": story.moral,
        "body": story.body,
        "output_webp": f"ru/wisdom-stories/illustrations/{stem}.webp",
        "prompt": prompt,
    }
...
def main() -> int:
    ...
    payload = {
        ...
        "text_rule": "On-image Russian must match Exact quotes exactly",
        "items": items,
    }
