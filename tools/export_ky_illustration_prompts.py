# -*- coding: utf-8 -*-
"""Export resume-friendly KY wisdom-story illustration prompts from DOCX.

Does not call any image API. Use when regenerating
`ky/wisdom-stories/illustrations/{stem}.webp` so on-image Kyrgyz text matches
the story exactly.

  python tools/export_ky_illustration_prompts.py --pending --limit 5
  python tools/export_ky_illustration_prompts.py --stem baklava
...
PROGRESS = ROOT / "docs" / "reviews" / "ky-illustration-regen-progress.json"
DOCX_DIR = ROOT / "ky" / "wisdom-stories"
...
STYLE = (
    ...
    "No English text. Every letter of Kyrgyz text on the image must match "
    "the Exact quotes below character-for-character (spelling, punctuation, "
    "capitalization). Do not paraphrase, translate, or invent dialogue."
)
...
def build_prompt(stem: str) -> dict:
    ...
    return {
        ...
        "moral": story.moral,
        "body": story.body,
        "output_webp": f"ky/wisdom-stories/illustrations/{stem}.webp",
        "prompt": prompt,
    }
...
def main() -> int:
    ...
    if not stems:
        # default: next pending from progress
        stems = [
            e["stem"]
            for e in log.get("entries") or []
            if e.get("status", "pending") == "pending"
    ...
    payload = {
        ...
        "text_rule": "On-image Kyrgyz must match Exact quotes exactly",
        "items": items,
    }
