# -*- coding: utf-8 -*-
"""Rebuild Kyrgyz Discoveries article bodies from Age10-14 Word sources.
...
article ids, category chrome, image paths, TOC structure, and URLs.
  ...
  python tools/rebuild_ky_discovery_from_docx.py
  python tools/rebuild_ky_discovery_from_docx.py --dry-run
"""
...
DOCX_DIR = ROOT / "ky" / "discovery-articles" / "Age10-14"
PAGE = ROOT / "ky" / "discoveries" / "discoveries-and-inventions.html"
BODY = ROOT / "tools" / "inventions" / "ky-body.html"
...
META_SPLIT_RE = re.compile(r"^(Мезгил:\s*.+?)(?:\s*\|\s*(.+))?$", re.S)
...
FIGURES_LABEL = "Негизги инсандар:"
ALT_PREFIX = "Сүрөт:"
...
def parse_docx(path: Path) -> ParsedArticle:
    ...
    i = 1
    if i < len(rows) and CATEGORY_RE.match(rows[i][0]) and not rows[i][0].startswith(PERIOD_PREFIX):
        parsed.category = rows[i][0]
        i += 1
    if i < len(rows) and rows[i][0].startswith(PERIOD_PREFIX):
    ...
    while i < len(rows) and rows[i][0] != FACTS_HEAD and not is_section_heading(rows[i][0]):
        summary_parts.append(rows[i][0])
        i += 1
    parsed.summary = " ".join(summary_parts).strip()
    if i < len(rows) and rows[i][0] == FACTS_HEAD:
        i += 1
    while i < len(rows) and rows[i][0] != REFS_HEAD and not is_section_heading(rows[i][0]):
        parsed.facts.append(rows[i][0])
        i += 1

    while i < len(rows) and rows[i][0] != REFS_HEAD:
        text, _style = rows[i]
    ...
    if i < len(rows) and rows[i][0] == REFS_HEAD:
        i += 1
    current_title = ""
...
def render_article(parsed: ParsedArticle, article_id: str, number: str, media_html: str, fallback_figures: str) -> str:
    ...
    media = IMG_ALT_RE.sub(rf"\1{ALT_PREFIX} {title}\3", media_html, count=1)
    names = parsed.figures_names
    if names:
        figures = (
            f'<p class="inventions-entry-visual-figures"><strong>{html.escape(FIGURES_LABEL)}</strong> '
    ...
    facts_html = f'<div class="inventions-key-facts"><h3>{html.escape(FACTS_HEAD)}</h3><ul>{facts}</ul></div>'
    sections = []
    ...
    return (
        ...
        f'<span aria-hidden="true" class="inventions-entry-num">{html.escape(number)}</span>'
